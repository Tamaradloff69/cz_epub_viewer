...
var head = contents.document.head;
    ...
    style.id = "custom-font-style"; // Give it a unique ID so we can find it later
...
function applyReaderStyles(fontFamily, fontSize) {
    ...
    if (rendition) {
        // Map the font family name to its file.
        // Ensure these file names EXACTLY match the files in your assets/fonts folder.
        var fontFiles = {
            'Nunito': 'Nunito-Regular.ttf',
            'Playfair Display': 'PlayfairDisplay-Regular.ttf',
            'Open Dyslexic': 'OpenDyslexic-Regular.otf'
        };

        var fontFile = fontFiles[fontFamily];
        var fontFormat = (fontFile && fontFile.endsWith('.otf')) ? 'opentype' : 'truetype';

        if (!fontFile) {
            console.log("Error: Font file not found for family: " + fontFamily);
            return;
        }

        // Create the complete CSS content, including the @font-face rule.
        // The path must be relative to the HTML file's location.
        var cssContent = `
            @font-face {
        ...
        // Register and select the theme to apply it.
        // Using a new theme name each time ensures it reapplies.
        var themeName = "custom-" + new Date().getTime();
        rendition.themes.register(themeName, {
            "*": {}, // Base rule
            "rules": cssContent // Inject our raw CSS
        });
        rendition.themes.select(themeName);

        console.log("Custom styles and font-face injected.");
    } else {
