...
function loadBook(data, cfi, manager, flow, spread, snap, allowScriptedContent, direction, useCustomSwipe, backgroundColor, foregroundColor) {
  ...
  rendition.hooks.content.register((contents) => {
    // This hook injects the @font-face rules for all your fonts
    // as soon as a chapter's content is loaded.
    var head = contents.document.head;
    var style = contents.document.createElement("style");
    style.id = "custom-font-faces";

    // Add all your @font-face rules here
    style.innerHTML = `
        @font-face {
            font-family: 'Nunito';
            src: url('https://appassets.androidplatform.net/assets/packages/flutter_epub_viewer/lib/assets/webpage/fonts/Nunito-Regular.ttf') format('truetype');
        }
        @font-face {
            font-family: 'Playfair Display';
            src: url('https://appassets.androidplatform.net/assets/packages/flutter_epub_viewer/lib/assets/webpage/fonts/PlayfairDisplay-Regular.ttf') format('truetype');
        }
        @font-face {
            font-family: 'Open Dyslexic';
            src: url('https://appassets.androidplatform.net/assets/packages/flutter_epub_viewer/lib/assets/webpage/fonts/OpenDyslexic-Regular.otf') format('opentype');
        }
    `;

    head.appendChild(style);

    // Your existing swipe detection code (if you need it)
    if (useCustomSwipe) {
...
// 2. The function that Dart will call with the Base64 font data
function setupCustomFonts(fontMap) {
  var cssString = "";
  for (const fontFamily in fontMap) {
    if (fontMap.hasOwnProperty(fontFamily)) {
      const base64 = fontMap[fontFamily];
      // Determine format from the name. You can make this smarter if needed.
      const format = (fontFamily === 'Open Dyslexic') ? 'opentype' : 'truetype';
      const mime = (fontFamily === 'Open Dyslexic') ? 'font/otf' : 'font/ttf';
  ...
  FONT_FACE_CSS = cssString; // Store the generated CSS globally
  console.log("Custom font faces have been prepared.");
}


// 3. REPLACE your existing rendition.hooks.content.register block with this
rendition.hooks.content.register((contents) => {
  // This hook now injects the dynamically created @font-face rules
  var head = contents.document.head;
  var style = contents.document.createElement("style");
  style.id = "custom-font-faces";
  style.innerHTML = FONT_FACE_CSS; // Use the CSS string from our global variable
  head.appendChild(style);

  // Your existing swipe detection code...
  if (useCustomSwipe) {
    const el = contents.document.documentElement;
    if (el) {
      detectSwipe(el, function (el, direction) {
